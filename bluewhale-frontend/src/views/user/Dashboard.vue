...
import {getRecommendationOfCurrentUser} from "../../api/recommend.ts";
...
function getUserRecommendation(){
  getRecommendationOfCurrentUser().then(
      res=>{
        if(res.data.code==='000'){
          recommendProductList.value=res.data.result
        }else{
          ElMessage({
            message:"无法获得推荐",
            type:"error",
  ...
  <el-main class="main-container">
    <el-card class="aside-card">
      ...
      >
        ...
        <el-descriptions-item label="收货地址" v-if="role === 'CUSTOMER'">
          <router-link to="/allcandidate" v-slot="{navigate}">
            <el-button @click="navigate" size="small">
              去编辑
            </el-button>
          </router-link>
        </el-descriptions-item>
